!(function () {
  ...
  ((window.TrackFather = {
    ...
    init: function () {
      try {
        if (!window.TrackFatherConfig || !window.TrackFatherConfig.workspace_id)
          return void n(
            "TrackFatherConfig não definido ou workspace_id ausente",
          );
    ...
    showConsoleBranding: function () {
      try {
        if (window.__trackfather_branded) return;
        ((window.__trackfather_branded = !0),
          ...
          console.log(
            "%c🚀 Este site usa TrackFather - Marketing Attribution & Analytics",
            "color: #4F46E5; font-size: 14px; font-weight: bold; padding: 8px 0;",
          ),
          console.log(
            "%c📊 Plataforma completa de atribuição de marketing para Telegram",
            "color: #6B7280; font-size: 12px;",
          ),
          console.log(
            "%c🔗 Conheça mais em: https://trackfather.com",
    ...
    initFacebookPixel: function (e) {
      try {
        if (window.fbq)
          return (
            t("Facebook Pixel já está inicializado"),
            window.fbq("init", e),
        ...
        document.addEventListener(
          ...
          (e) => {
            try {
              ...
              for (let e = 0; e < 3 && t; e++) {
                if (
                  (t.classList && t.classList.contains("trackfather")) ||
                  (t.className && t.className.includes("trackfather"))
                ) {
    ...
    sendPageView: function () {
      try {
        if (sessionStorage.getItem(r)) return;
        (this.sendEvent("page_view", {}), sessionStorage.setItem(r, "true"));
      } catch (e) {
    ...
    sendButtonClick: function (e) {
      try {
        const t = {
          ...
          element_text: e.textContent
            ? e.textContent.trim().substring(0, 100)
            : null,
          element_href: e.href || e.getAttribute("href") || null,
          element_type: e.type || null,
          element_value: e.value || null,
        };
        (this.showRedirectLoading(null), this.sendEvent("click_button", t));
      } catch (e) {
        (this.hideRedirectLoading(), n("Erro no sendButtonClick:", e));
      }
    },
          ...
          .then((t) => {
            (t && t.trackfather_id && this.saveId(t.trackfather_id),
              "click_button" === e &&
                t &&
                (t.redirect_url
                  ? ((this.currentRedirectUrl = t.redirect_url),
                    this.handleInstantRedirect(t.redirect_url))
                  : t.invite_link
                    ? ((this.currentRedirectUrl = t.invite_link),
                      this.handleInstantRedirect(t.invite_link))
                    : this.hideRedirectLoading()));
          })
          .catch((t) => {
            if ("click_button" === e) {
              const e = t.message || "";
              e.includes("UNAUTHORIZED_URL") ||
              e.includes("não autorizada") ||
              e.includes("Domain not authorized") ||
              e.includes("403")
      ...
      return new Promise((n, i) => {
        try {
          const o = new XMLHttpRequest(),
            ...
            r = setTimeout(() => {
              (o.abort(), i(new Error("Request timeout")));
            }, a);
            ...
            if (4 === o.readyState) {
              ...
              try {
                if (o.status >= 200 && o.status < 300) {
                  ...
                  n(e);
                } else i(new Error(`HTTP ${o.status}: ${o.responseText}`));
              } catch (e) {
                i(e);
              }
            }
    ...
    saveId: function (e) {
      try {
        (this.saveCookie(i, e), localStorage.setItem(i, e));
      } catch (e) {
    ...
    saveCookie: function (e, t) {
      try {
        ...
        const r = [
          `${e}=${t}; expires=${n.toUTCString()}; path=/; SameSite=Lax${i ? "; Secure" : ""}`,
          a
            ? `${e}=${t}; expires=${n.toUTCString()}; path=/; domain=${a}; SameSite=Lax${i ? "; Secure" : ""}`
            : null,
          `${e}=${t}; expires=${n.toUTCString()}; path=/`,
          `${e}=${t}; max-age=31536000; path=/`,
        ].filter(Boolean);
        let s = !1;
        ...
        if (!s)
          throw new Error(
            "Falha em todas as tentativas de configuração de cookie",
          );
      } catch (e) {
      ...
      try {
        ...
        if (o) {
          ...
          o.addEventListener("click", function (t) {
            (t.preventDefault(),
              e.currentRedirectUrl &&
                e.handleInstantRedirect(e.currentRedirectUrl));
          });
        }
      } catch (e) {
    ...
    handleInstantRedirect: function (e) {
      try {
        if (!e || "string" != typeof e) return void this.hideRedirectLoading();
        let t;
        try {
          t = new URL(e).href;
        } catch (n) {
        ...
        (this.hideRedirectLoading(), n("Erro no redirecionamento:", t));
        try {
          window.open(e, "_self");
        } catch (e) {
